...
vec3 CalculateReflection () {
	vec3 D = normalize(vec3(FragPos.x, FragPos.y + 20.0, FragPos.z) - eyePosition);
	vec3 Refl = reflect(D, fragNormal);
	return vec3(texture(tex_skybox, Refl));
}
...
void main()
{
	...
	// reflection 
	vec3 testRefra = CalculateReflection();
	tempColor = (1.0 - reflectionCof) * tempColor + vec4(testRefra, 1.0) * reflectionCof;
