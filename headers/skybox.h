...
void CreateSkyBox()
{
	...
	// Program
	programSkybox = CProgram("shaders/skybox-vs.glsl", "shaders/skybox-fs.glsl");
...
void DrawSkyBox(CCamera cview)
{
	// Specjalny potok dla SkyBoxa
	programSkybox.UseProgram();

	// Przeskalowanie boxa i przeslanie macierzy rzutowania
	glm::mat4 matProjView = cview.matProj * cview.matView * glm::scale(glm::mat4(1), glm::vec3(60.0, 60.0, 60.0));

	programSkybox.SendUniformMatrix4fv("matProjView", matProjView);

	// Aktywacja tekstury CUBE_MAP
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_CUBE_MAP, idCubeTexture);
	programSkybox.SendUniform1i("tex_skybox", 0);
