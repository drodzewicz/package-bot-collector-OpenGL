...
public:
	...
	float getAltitute(glm::vec3 point)
	{
		...
		for (CTriangle it : triangles)
		{
			if (it.isInside(glm::vec2(point.x, point.z)))
			{
				float calculatedAltitude = it.calulateY(glm::vec2(point.x,point.z));
				ttl.push_back(calculatedAltitude);
				float altitudeDiff = abs(point.y - calculatedAltitude);
				if(altitudeDiff < abs(point.y - altitude)) {
					altitude = calculatedAltitude;
					if(altitudeDiff < 0.001) {
						break;
					}
				}
			}
